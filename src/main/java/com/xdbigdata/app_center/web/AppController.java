...
public class AppController {
    ...
    public CommonResult findAll(HttpSession httpSession, @RequestBody RoleAndInfoDto roleAndInfoDto) {
        ...
        //查询到选择的selectedRolesNames
        List<String> selectedRolesNames = roles.stream().map(r -> r.getName()).collect(Collectors.toList());
        List<String> roleNames = roleAndInfoDtoInSession.getRoleList().stream().map(r -> r.getName()).collect(Collectors.toList());
